<div align="center"><center>

## Command Keys

[![Environment](https://img.shields.io/badge/Environment-Client-purple)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/65UyswbY?label=Latest%20Minecraft&color=%2300AF5C)](https://modrinth.com/mod/command-keys/versions)

[![Loader](https://img.shields.io/badge/Loader-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Loader-Quilt-9115ff?logo=data:image/png;base64,)](https://quiltmc.org/)
[![Loader](https://img.shields.io/badge/Loader-Forge-1d2d41?logo=data:image/png;base64,)](https://files.minecraftforge.net)
[![Loader](https://img.shields.io/badge/Loader-NeoForge-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Modrinth Downloads](https://img.shields.io/modrinth/dt/65UyswbY?label=Modrinth%20Downloads&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/mod/command-keys)

Allows the user to send pre-written messages and commands using single and dual-step hotkeys.

</center></div>

### Features and Usage

#### Mono-Key Activation

- Access the config screen from the mod list, or by pressing the config key (default: K) in-game and
  clicking "Mono-Key Options"
- Use the GUI to add/edit/remove key-message pairs. If a key is assigned to another function, the conflict
  will be shown in a tooltip, and the command key will be ignored until the conflict is resolved.
- Once you have bound the message to an available key, simply press the key in-game to send the message.

<img alt="Mono-Key Options" src="https://i.postimg.cc/tgczwW21/2023-11-25-12-13-30.png" width="637">

#### Dual-Key Activation

- If you do not have enough available keyboard keys to bind every message, you can use this option.
- Access the config screen by pressing the config key (default: K) in-game, or open the Mono-Key screen
  from the mod list and click "Dual-Key Options".
- Use the GUI to add/edit/remove key-message pairs. These keys do not conflict with other assignments
  (unlike mono-keys), so you can assign keys such as W, A, S, D.
- To send dual-key messages, first press the config key (default: K) in-game, then press the message key.
  For example, pressing K then A will send the /say command shown below.

<img alt="Configuration" src="https://i.postimg.cc/9f0pLJt5/2023-11-25-12-13-18.png" width="637">

#### Chaining Commands

- If you want a command key to send multiple commands sequentially, you can enter each command into the
  same message field, separated by a pair of commas.
- See the last entry in the image above.

### Credits

- CommandKeys includes derivative work of code from the following projects:
    - [Sodium](https://github.com/CaffeineMC/sodium-fabric/)

### License

Except where otherwise stated, CommandKeys source files are provided under the terms of the GNU LGPL-3.0-only 
open-source license. For more information, refer to the provided LICENSE file.

### Contact

[![Discord](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord&color=%235865F2)](https://discord.gg/sSgQ9AF4mU)

[![GitHub Issues](https://img.shields.io/github/issues/NotRyken/CommandKeys?logo=github&label=Issues)](https://github.com/NotRyken/CommandKeys/issues)