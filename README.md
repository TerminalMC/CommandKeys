...
[![Minecraft](https://img.shields.io/modrinth/game-versions/65UyswbY?label=Minecraft&color=%2300AF5C)](https://modrinth.com/mod/command-keys/versions)
...
[![Modrinth](https://img.shields.io/modrinth/dt/65UyswbY?label=Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/mod/command-keys)
[![CurseForge](https://img.shields.io/curseforge/dt/873388?label=CurseForge&logo=curseforge&logoColor=%23F16436)](https://www.curseforge.com/minecraft/mc-mods/command-keys)

Command Keys provides client-side configurable support for sending messages and commands using single and dual-key hotkeys.
...
- Access the config screen from the mod list, or by pressing the config key (default: K) in-game and
clicking "Mono-Key Options"
- Use the GUI to add/edit/remove key-message pairs. If a key is assigned to another function, the conflict
will be shown in a tooltip, and the command key will be ignored until the conflict is resolved.
- Once you have bound the message to an available key, simply press the key in-game to send the message.

<img alt="Mono-Key Options" src="https://i.postimg.cc/tgczwW21/2023-11-25-12-13-30.png" width="637">

#### Dual-Key Activation

- If you do not have enough available keyboard keys to bind every message, you can use this option.
- Access the config screen by pressing the config key (default: K) in-game, or open the Mono-Key screen
from the mod list and click "Dual-Key Options".
- Use the GUI to add/edit/remove key-message pairs. These keys do not conflict with other assignments 
(unlike mono-keys), so you can assign keys such as W, A, S, D.
- To send dual-key messages, first press the config key (default: K) in-game, then press the message key.
For example, pressing K then A will send the /say command shown below.

<img alt="Configuration" src="https://i.postimg.cc/9f0pLJt5/2023-11-25-12-13-18.png" width="637">

#### Chaining Commands

- If you want a command key to send multiple commands sequentially, you can enter each command into the 
same message field, separated by a pair of commas.
- See the last entry in the image above.
